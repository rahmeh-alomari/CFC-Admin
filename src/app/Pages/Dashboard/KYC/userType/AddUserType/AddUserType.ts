...
export class AddUserTypeComponent implements OnInit {
  ...
  errorHandler(){
    ...
    getUserTypeDetails(){
        this.kycService.getUserType(this.id).subscribe((res:any)=>{
            if(res.status){
    ...
    add(data){
        this.kycService.addUserType(data).subscribe((res:any)=>{
            this.load=false;
            if(res.status){
                this.toastr.successToastr(this.LANG.User_Type_added_successfully);
                this.location.back();
                return
            }
    ...
    update(data){
        this.kycService.updateUserType(data).subscribe((res:any)=>{
            this.load=false;
            if(res.status){ 
                this.toastr.successToastr(this.LANG.User_Type_updated_successfully);
                this.location.back();
                return
            }
            this.toastr.warningToastr(res.message);
